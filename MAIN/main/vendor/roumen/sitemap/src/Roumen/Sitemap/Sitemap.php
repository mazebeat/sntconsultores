...
class Sitemap
{
	...
	public function add($loc, $lastmod = null, $priority = null, $freq = null, $images = array(), $title = null, $translations = array())
	{
		...
		$this->model->setItems(array('loc'          => $loc,
		                             'lastmod'      => $lastmod,
		                             'priority'     => $priority,
		                             'freq'         => $freq,
		                             'images'       => $images,
	...
	{
		$this->model->setSitemaps(array('loc'     => $loc,
		                                'lastmod' => $lastmod));
	}
	...
	public function generate($format = 'xml')
	{
		...
		$channel = array('title' => $this->model->getTitle(),
		...
		switch ($format) {
			case 'ror-rss':
				return array('content' => View::make('sitemap::ror-rss', array('items' => $this->model->getItems(), 'channel' => $channel))->render(),
				             'headers' => array('Content-type' => 'text/rss+xml; charset=utf-8'));
			case 'ror-rdf':
				return array('content' => View::make('sitemap::ror-rdf', array('items' => $this->model->getItems(), 'channel' => $channel))->render(),
				             'headers' => array('Content-type' => 'text/rdf+xml; charset=utf-8'));
			case 'html':
			...
			case 'sitemapindex':
				return array('content' => View::make('sitemap::sitemapindex', array('sitemaps' => $this->model->getSitemaps()))->render(),
