...
class Compiler
{
		...
		while (false !== $f = readdir($h)) {
			if (false === strpos($f, '.') && is_file($map_dir . $f)) {
				...
				foreach ($data as $data) {
					...
					$data[1] = $data[1] > 255 ? chr($data[1] >> 8) . chr($data[1] % 256) : chr($data[1]);
			...
			if (0 === strpos($f, 'bestfit') && preg_match('/^bestfit\d+\.txt$/D', $f)) {
				...
				while (false !== $s = fgets($h)) {
					...
					if (isset($s[1])) {
						...
						$s[1] = substr($s[1], 2);
						$s    = array_map('hexdec', $s);
						$s[1] = $s[1] > 255 ? chr($s[1] >> 8) . chr($s[1] % 256) : chr($s[1]);
	...
	public static function unicodeMaps($out_dir)
	{
		...
		$caseFolding = array(array_keys($caseFolding['F']),
