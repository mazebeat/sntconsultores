...
class Iconv
{
	...
	public static function iconv_get_encoding($type = 'all')
	{
		...
		return array('input_encoding'    => self::$input_encoding,
	...
	public static function iconv_mime_encode($field_name, $field_value, $pref = INF)
	{
		...
		foreach ($chars as $c) {
			...
			$o = $Q ? $c = preg_replace_callback('/[=_\?\x00-\x1F\x80-\xFF]/', array(__CLASS__, 'qp_byte_callback'), $c) : base64_encode($line_data . $c);
